...
export default withApiAuthRequired(
	async (req: NextApiRequest, res: NextApiResponse) => {
		...
		if (!req.body.name || !req.body.album) {
			return res
				.status(400)
				.json({ message: 'Must contain artist name and album name.' });
		}
		...
		const response = await fetch(
			`http://ws.audioscrobbler.com/2.0/?method=album.getinfo&artist=${encodeURIComponent(
				req.body.name
			)}&album=${encodeURIComponent(req.body.album)}&api_key=${
		...
		const albumCover =
			data.message === undefined ? data.album.image[3]['#text'] : false;
